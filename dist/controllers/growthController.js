...
exports.getCreatorGrowth = void 0;
const db_1 = __importDefault(require("../config/db"));
const getCreatorGrowth = async (req, res) => {
    if (!req.user)
    ...
    try {
        ...
        const [followers, posts, totalViews, totalLikes, totalShares, totalComments, activeHoursRaw] = await Promise.all([
            ...
            db_1.default.post.count({ where: { userId, type: 'reel' } }),
            db_1.default.postView.count({ where: { post: { userId } } }),
            db_1.default.like.count({ where: { post: { userId } } }),
            db_1.default.post.aggregate({ where: { userId }, _sum: { shareCount: true } }).then((r) => r._sum.shareCount || 0),
            db_1.default.comment.count({ where: { post: { userId } } }),
      ...
      `,
        ...
        const posts30d = await db_1.default.post.count({ where: { userId, createdAt: { gte: since30 } } });
        const recentPosts = await db_1.default.post.findMany({
        ...
        if (activeHoursRaw && activeHoursRaw.length > 0) {
            const peak = activeHoursRaw[0];
...
exports.getCreatorGrowth = getCreatorGrowth;
