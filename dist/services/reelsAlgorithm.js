...
exports.getRecommendedReels = getRecommendedReels;
exports.getColdStartReels = getColdStartReels;
 ...
 */
function calcEngagementScore(reel) {
    ...
    const weightedActions = (likes * ENGAGEMENT_WEIGHTS.like) +
 ...
 */
function calcWatchTimeScore(avgWatchDuration, totalViews, maxWatchDuration) {
    if (totalViews === 0)
        return 40; // No data = below neutral
    ...
    if (completionRate < 0.25)
        return completionRate * 80; // 0-20
    if (completionRate < 0.50)
        return 20 + ((completionRate - 0.25) * 100); // 20-45
    if (completionRate < 0.70)
        return 45 + ((completionRate - 0.50) * 100); // 45-65
    if (completionRate < 0.90)
        return 65 + ((completionRate - 0.70) * 100); // 65-85
 ...
 */
function calcTrendingScore(recentLikes, recentViews, ageHours) {
    if (ageHours > 168)
        return 0; // Older than 7 days = no trending boost
    // Velocity = recent engagement per hour
    const recentHours = Math.min(ageHours, 6); // Look at last 6 hours max
    if (recentHours === 0)
        return 80; // Brand new = high trending potential
 ...
 */
function calcAffinityScore(isFollowing, likedCreatorPosts, commentedCreatorPosts, viewedCreatorPosts) {
    let score = 0;
    if (isFollowing)
        score += 30; // Following = base affinity
    // Interaction depth
 ...
 */
function calcFreshnessScore(ageHours) {
 ...
 */
function calcContentPreferenceScore(reel, userCategoryPrefs, // category -> weighted watch count
userTopCreators, // creator IDs user engages with most
userHashtagPrefs) {
    let score = 0;
...
async function getRecommendedReels({ userId, limit, cursor, excludeIds = [] }) {
    const now = new Date();
    ...
    const userLikes = await db_1.prisma.like.findMany({
    ...
    const allUserViews = await db_1.prisma.postView.findMany({
        where: { userId, createdAt: { gte: thirtyDaysAgo } },
    ...
    const viewedPostIds = allUserViews.map((v) => v.postId);
    const viewedPosts = await db_1.prisma.post.findMany({
    ...
    const userCategoryPrefs = {};
    viewedPosts.forEach((p) => {
    ...
    const likedPosts = await db_1.prisma.post.findMany({
    ...
    likedPosts.forEach((p) => {
        const cat = (p.category || '').toLowerCase();
        if (cat)
            userCategoryPrefs[cat] = (userCategoryPrefs[cat] || 0) + 3; // Like = 3x weight boost
    });
    ...
    const userTopCreators = new Set(Object.entries(creatorEngagement)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 20)
        .map(([id]) => id));
    // Hashtag preferences — also watch-based
    const userHashtagPrefs = {};
    viewedPosts.forEach((p) => {
        const duration = watchDurationMap[p.id] || 1;
        const watchWeight = duration < 3 ? 0.3 : duration > 10 ? 1.5 : 1.0;
        const tags = ((p.caption || '').match(/#(\w+)/g) || []).map((t) => t.replace('#', '').toLowerCase());
        tags.forEach((tag) => { userHashtagPrefs[tag] = (userHashtagPrefs[tag] || 0) + watchWeight; });
    });
    likedPosts.forEach((p) => {
        const tags = ((p.caption || '').match(/#(\w+)/g) || []).map((t) => t.replace('#', '').toLowerCase());
        tags.forEach((tag) => { userHashtagPrefs[tag] = (userHashtagPrefs[tag] || 0) + 2; });
    });
    ...
    const visibleCandidates = candidates.filter((r) => {
        if (r.userId === userId)
            return true; // Always show own reels
    ...
    const creatorIds = [...new Set(visibleCandidates.map((r) => r.userId))];
    // Get posts by these creators that user has liked
    const userLikesOnCreators = await db_1.prisma.like.findMany({
        where: { userId, post: { userId: { in: creatorIds } } },
        select: { post: { select: { userId: true } } },
    });
    // Count likes per creator
    const likesPerCreator = {};
    userLikesOnCreators.forEach((l) => {
    ...
    const userCommentsOnCreators = await db_1.prisma.comment.findMany({
        where: { userId, post: { userId: { in: creatorIds } } },
        select: { post: { select: { userId: true } } },
    });
    const commentsPerCreator = {};
    userCommentsOnCreators.forEach((c) => {
        const cid = c.post.userId;
        commentsPerCreator[cid] = (commentsPerCreator[cid] || 0) + 1;
    });
    // Get creator post counts (for new creator boost)
    const creatorPostCounts = await db_1.prisma.post.groupBy({
        by: ['userId'],
        where: { userId: { in: creatorIds }, type: 'reel' },
        _count: { id: true },
    });
    const postCountMap = {};
    creatorPostCounts.forEach((c) => { postCountMap[c.userId] = c._count.id; });
    // 7. Score each reel
    const scoredReels = visibleCandidates.map((reel) => {
        ...
        const recentViews = (reel.postViews || []).filter((v) => new Date(v.createdAt) >= sixHoursAgo).length;
        // Filter likes by createdAt if available, otherwise estimate from total
        const recentLikes = (reel.likes || []).filter((l) => l.createdAt ? new Date(l.createdAt) >= sixHoursAgo : false).length || Math.ceil((reel._count?.likes || 0) * Math.min(6 / Math.max(ageHours, 1), 1));
        ...
        const viewedCount = (reel.postViews || []).filter((v) => v.userId === userId).length;
        ...
        if (creatorReelCount <= 3)
            bonusMultiplier *= 1.6; // Very new creator (≤3 reels) = 1.6x
        else if (creatorReelCount <= 8)
            bonusMultiplier *= 1.3; // Growing creator (4-8 reels) = 1.3x
        // ★ COMPLETION RATE SUPER BOOST — Instagram 2026 heavily rewards 70%+ completion
        const avgWatchForBoost = watchDurations.length > 0
            ? watchDurations.reduce((a, b) => a + b, 0) / watchDurations.length : 5;
        const estLength = maxWatch && maxWatch > 3 ? maxWatch : 15;
        const completionForBoost = avgWatchForBoost / estLength;
        if (completionForBoost >= 0.8)
            bonusMultiplier *= 1.4; // 80%+ completion = big boost
        else if (completionForBoost >= 0.7)
            bonusMultiplier *= 1.2; // 70%+ = moderate boost
        ...
        if (shareRate > 0.05)
            bonusMultiplier *= 1.5; // 5%+ share rate = viral potential
        else if (shareRate > 0.02)
            bonusMultiplier *= 1.2; // 2%+ = good shareability
        // ─── DEMOTION SIGNALS (Instagram 2026: reduce reach for low quality) ───
        // Already viewed penalty (seen it before = lower priority)
        const userViewed = (reel.postViews || []).some((v) => v.userId === userId);
        if (userViewed)
            bonusMultiplier *= 0.25; // Heavy penalty for already-seen
        // Already liked = don't show again
        if (userLikedPostIds.has(reel.id))
        ...
        const rawScore = (engagementScore * WEIGHTS.engagement) +
 ...
 */
async function getColdStartReels(userId, limit) {
    const blocks = await db_1.prisma.block.findMany({
        where: { OR: [{ blockerId: userId }, { blockedId: userId }] },
        select: { blockerId: true, blockedId: true },
    });
