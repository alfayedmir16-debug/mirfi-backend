...
export const getMe = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    res.status(200).json({
      ...
      hiddenStoryFrom: (user as any).hiddenStoryFrom || [],
      stats: {
  ...
  try {
    ...
    const updatedUser = await prisma.user.update({
      ...
      data: {
        ...
        showGender: showGender !== undefined ? showGender : (undefined as any),
...
export const requestAccountDeletion = async (req: Request, res: Response) => {
  try {
    ...
    await sendResetCodeEmail(email, deletionCode, 'delete');
...
export const confirmAccountDeletion = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      await prisma.postView.deleteMany({ where: { userId: user.id } });
      await prisma.save.deleteMany({ where: { userId: user.id } });
